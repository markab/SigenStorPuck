...
//
// This is one board profile. board_config.h selects between the profiles at
// compile time; nothing else in the tree includes this file directly — see
// CLAUDE.md. Nothing else in the tree carries a hardware constant.
//
...
// The compile-time board slug. It names this variant's release directory on
// GitHub Pages and its OTA manifest, and guards against flashing another
// board's image (see updater.cpp). Never reused across boards.
#define PUCK_BOARD_SLUG "puck-1.75"
#define PUCK_BOARD_NAME "Waveshare ESP32-S3-Touch-AMOLED-1.75"

// Shape fact, so shared code can branch on a named constant rather than an
// #ifdef or an "is it the round one?" check — the data_source_capabilities()
// ethos, applied to geometry at compile time.
static constexpr bool PUCK_DISPLAY_ROUND = true;
